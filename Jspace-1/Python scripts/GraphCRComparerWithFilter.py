#Same script as above with totalbudget as 1000(changed to 800 as total number of edges:828) initialtraining still 100 increment budget 50
#Now also changing filtered_df = results_df[results_df[score_col] >= 0.1] to filtered_df = results_df[results_df[score_col] >= 0.5]
...
def prep_data(folder, score_col):
    filtered_df = results_df[results_df[score_col] >= 0.5] #Input Threshold in section 6.2.2 in the paper 
...
for name, folder in [("Baseline", path_base), ("JSpace", path_jspc)]:
    ...
    try:
        al_famer.evaluate(
            ...
            total_budget=800,  #changed to 800 from 100 to ignore the crash as total number of edges:828 for jspace run.. with filter >=0.5
            output=out_csv
        )
